...
TITLE.Text = "| ADHC AUTOFARM - @ali.203"
...
MAP.Image = "http://www.roblox.com/asset/?id=17168506163"
...
if (_G.AutofarmSettings.Credits ~= "ADHC autofarm - by ali.203") then 
...
local Log = function(Msg)
	print(os.date("%X").." | ADHC autofarm - "..Msg)
end
...
task.spawn(function()
	while true and task.wait(_G.AutofarmSettings.LogInterval * 60) do
        local s, e = pcall(function()
            SendLog(_G.AutofarmSettings.Webhook, {Player.Name, Player.UserId, WALLET.Text, PROFIT.Text, TIMER.Text, BROKEN.Text, "| ADHC autofarm by @ali.203"})
        end)
        if (e) then 
...
task.spawn(function()
	while true and task.wait() do 
		...
		Update()
		for i,v in pairs(Cashiers:GetChildren()) do 
			if ( (i == 23) or (i == 15) ) and distance(Vector3.new(-625, 10, -286), v.Head.Position) <= 20 then 
			...
			elseif ( (i == 24) or (i == 16) ) and distance(Vector3.new(-625, 10, -286), v.Head.Position) <= 20 then  -- THIS IS THE GAY ONE
